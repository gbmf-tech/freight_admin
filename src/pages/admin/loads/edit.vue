    ...
    <v-card flat>
        <div class="px-8 pt-8">
            <v-form @submit.prevent="submit" enctype="multipart/form-data">
                ...
                <v-radio-group v-model="Load.is_carrier" :column="false" @change="resetExecutor">
                ...
                <v-row>
                    <v-col cols="12" md="4">
                        ...
                        <text-field-component v-if="Load.executor" :value="Load.executor.name" readonly outlined flat hide-details/>
                        <autocomplete-component
                            ref="executor"
                            v-if="!Load.executor"
                            @setItem="setExecutor"
                ...
                <div class="">
                    <v-tabs v-model="Shipper.tab" background-color="transparent">
                        <v-tab v-for="(item, index) in Load.shippers" :key="index" :href="`#shipper-tab-${index}`">
                            Shipper {{ index + 1 }}
                            <v-icon v-if="Load.shippers.length > 1" dark class="ml-3" @click="removeTab('shippers', index)">mdi-window-close</v-icon>
                        </v-tab>
                        ...
                        <v-tab-item v-for="(item, index) in Load.shippers" :key="index" :value="'shipper-tab-' + index" class="bg-transparent">
                            <v-row class="mt-4">
                                <v-col cols="12" md="4">
                                    ...
                                    <text-field-component v-if="item.warehouse" :value="item.warehouse.name" readonly outlined flat hide-details/>
                                    <autocomplete-component
                                        v-if="!item.warehouse"
                                        @setItem="setShipper"
                                        :index="index"
                                ...
                                <v-col cols="12" md="4">
                                    ...
                                    <text-field-component
                                        v-if="item.warehouse"
                                        :value="item.warehouse.address + ', ' + item.warehouse.city + ', ' + item.warehouse.zip"
                                ...
                                <v-col cols="12" md="4">
                                    ...
                                    <datepicker-component :date="item.date" @setDate="shipperDate" :index="index"/>
                                </v-col>
                                <v-col cols="12" md="4">
                                    <label for="">Time</label>
                                    <timepicker-component @setTime="shipperTime" :index="index" :time="item.time"/>
                                </v-col>
                                <v-col cols="12" md="4">
                ...
                <div class="">
                    <v-tabs v-model="Consignee.tab" background-color="transparent">
                        <v-tab v-for="(item, index) in Load.consignees" :key="index" :href="`#consignee-tab-${index}`">
                            Consignee {{ index + 1 }}
                            <v-icon v-if="Load.consignees.length > 1" dark class="ml-3" @click="removeTab('consignees', index)">mdi-window-close</v-icon>
                        </v-tab>
                        ...
                        <v-tab-item v-for="(item, index) in Load.consignees" :key="index" :value="'consignee-tab-' + index" class="bg-transparent">
                            <v-row class="mt-4">
                                ...
                                <v-col cols="12" md="4">
                                    <label for="">Date</label>
                                    <datepicker-component :date="item.date" @setDate="consigneeDate" :index="index"/>
                                </v-col>
                                <v-col cols="12" md="4">
                                    <label for="">Time</label>
                                    <timepicker-component @setTime="consigneeTime" :index="index" :time="item.time"/>
                                </v-col>
                                <v-col cols="12" md="4">
                                    <label for="">Description</label>
                                    <text-field-component
...
export default {
    ...
    methods: {
        ...
        getLoad(){
            ...
            this.axios
                .get(api + this.$route.params.id).then(response => {
                    this.Load = Object.assign(this.Load, response.data);
                    response.data.items.filter((item) => {
                        if(item.additional){
                            item.additional = item.additional.map((itm) => {
                                return parseInt(itm, 10);
                            });
                        }

                        if(item.is_shipper){
                            this.Load.shippers.push(Vue.util.extend({}, item));
                        }else{
                            this.Load.consignees.push(Vue.util.extend({}, item));
                        }
                    })
                    response.data.files.filter((item) => {
                        if(item.deleted == 0 )
                            this.Load.documents.push(Vue.util.extend({}, item));
                    })
                    // this.Load.documents = response.data.files;

                    delete this.Load.items;
                });
        },
        ...
        removeTab(loadgroup, index){
            Vue.delete(this.Load[loadgroup], index);
        },
