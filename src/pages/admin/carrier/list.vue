<template>
    <div class="">
        <div class="px-8 pt-8">
            <div class="d-flex justify-space-between">
                <div class="cardtitle text-22-60" v-html="subtitle"></div>
                <v-btn large color="primary" class="ma-0 py-2" @click="$refs.dialogdatatable.dialogchange()" depressed>Add Carieer</v-btn>
            </div>
        </div>
        <data-table-default
            :headers="headers"
            :geturl="geturl"
            :action="action"
            :disablepagination="disablepagination"
            :editedItem="editedItem"
            ref="dialogdatatable">
            <template v-slot:inputs="{editItem}">
                <carrier-form :item="editItem" @update="editItem = Object.assign(editItem, $event)"/>
            </template>
        </data-table-default>
    </div>
</template>

<script>
import DataTableDefault from '@/components/Admin/DataTableDefault'
import CarrierForm from '@/components/page/carrierForm.vue'

export default {
    layout: 'admin',
    middleware: 'auth',
    components: {
        DataTableDefault,
        CarrierForm,
    },
    data: () => ({
        title: 'Carieers',
        subtitle: 'Carieers',
        headers: [
            { text: 'Business Name', value: 'name' },
            { text: 'Address', value: 'address' },
            { text: 'City', value: 'city' },
            { text: 'Contact Name', value: 'contact_name' },
            { text: 'Phone', value: 'phone' },
            { text: 'Actions', value: 'actions', sortable: false },
        ],
        editedItem: {
            id_city: null,
            city: null,
            zip: null,
            state: null
        },
        geturl: process.env.VUE_APP_AXIOS_DOMAIN + '/api/sudo/carriers/v1/carrier/',
        actionroute: 'admin.carriers.',
        action: ['edit', 'delete'],
        disablepagination: true,

    }),
    mounted(){
        this.$store.dispatch('title/setTitle', {title: this.title});
        this.$store.dispatch('title/setSubTitle', {cardtitle: this.subtitle});
    },
    methods: {
        setCity(val){
            this.$refs.dialogdatatable.setCity(val);
        }
    }
}
</script>
